...
const campList = document.getElementById("pendingCampsList");

onAuthStateChanged(auth, async (user) => {
    if (!user) { window.location.href = "login.html"; return; }
    
    // Security check: Verify if the user is actually an admin
...
async function loadPendingMakers() {
    ...
    makerList.innerHTML = snap.empty ? `<p class="text-slate-400 italic">No pending maker applications.</p>` : "";

    snap.forEach(dSnap => {
        const req = dSnap.data();
        
        // FIX: Use 'creatorName' and 'creatorEmail' instead of 'userName'/'userEmail'
        ...
        div.className = "bg-white p-6 rounded-2xl shadow-sm border-2 border-slate-100 flex justify-between items-center";
        ...
        div.innerHTML = `
            <div>
                ...
                <p class="text-[10px] text-blue-600 font-black uppercase tracking-wider">
                    🏛️ Org: ${orgName}
                </p>
                <p class="text-[10px] text-slate-400 font-bold">
                    🆔 ${req.govtIdType}: ${req.govtIdNumber}
                </p>
            </div>
...
async function loadPendingCamps() {
    ...
    campList.innerHTML = snap.empty ? `<p class="text-slate-400 italic">No camps awaiting approval.</p>` : "";
